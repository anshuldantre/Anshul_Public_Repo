...
class Burger:
    ...
    @classmethod
    def save(cls,data):
        query = """INSERT INTO burgers (name, bun, meat, calories, restaurant_id, created_at,updated_at) 
    ...
    def validate_burger(burger):
        ...
        if len(burger['name']) < 3:
            ...
            is_valid = False
        if len(burger['bun']) < 3:
            flash("Bun must be atleast 3 characters.")
            is_valid = False
        if len(burger['calories']) < 200:
            flash("Calories must be 200 or greater.")
            is_valid = False
        if len(burger['meat']) < 3:
            flash("Meat must be atleast 3 characters.")
            is_valid = False
        return is_valid